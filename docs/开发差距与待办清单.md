...
- ⚠️ B2 在线预订：已实现“登录后预订”；需求中“游客也可预订（不强制登录）”未满足
- ❌ B3 取消预订（顾客侧）：未实现（前台缺少取消入口与后端接口）
- ❌ B4 修改预订（顾客侧）：未实现（前台缺少修改入口与后端接口）
- ⚠️ B5 查询订单：已实现“会员中心查看订单列表”；缺少更完整的条件查询/订单状态解释
- ❌ B6 查看活动：未实现（目前仅有页面样式中的“最新活动”样式注释，缺少数据与页面内容）

建议开发任务：

1. 前台新增“我的订单”操作：取消预订、修改预订（改日期/房型/联系人信息等）
2. 预订状态模型补齐：至少区分“待确认/已确认/已取消/已入住/已完成”
3. 活动模块（见 3.3）完成后，前台首页展示活动列表/详情
4. 如需满足“游客可预订”：支持未登录下单（或引导游客一键注册后下单），并在报告中说明取舍
